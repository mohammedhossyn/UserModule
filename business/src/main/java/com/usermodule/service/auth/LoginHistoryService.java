...
public class LoginHistoryService {
    ...
    public void add(@NonNull Long systemUserId, String username, String hostName, String hostAddress, String requestIp) {
        log.debug("LoginHistoryService.add started");
        var user = userRepository.findById(systemUserId);
        if (user.isPresent()) {
    ...
    public Page<LoginHistoryResponseDTO> findBySystemUserId(@NonNull Pageable pageable, @NonNull Long id) {
        log.debug("LoginHistoryService.findBySystemUserId started");
        return userRepository.findById(id).map(systemUserEntity -> loginHistoryRepository
                .findByUser(pageable, systemUserEntity)
