...
public class RoleService {
    ...
    private final PermissionRepository systemPermissionRepository;
        ...
        try {
            ...
            if (!roleRepository.existsByName(roleAddRequestDTO.name())) {
                ...
                if (roleAddRequestDTO.permissionsId() != null && !roleAddRequestDTO.permissionsId().isEmpty()) {
                    ...
                    Set<PermissionEntity> systemPermissionEntities = new HashSet<>();
                    for (Long id : roleAddRequestDTO.permissionsId()) {
                        var permission = systemPermissionRepository.findById(id);
                        permission.ifPresent(systemPermissionEntities::add);
                    }
                    roleEntity.setPermissions(systemPermissionEntities);
                }
                ...
                throw BusinessException.builder().name("add role").message("err.systemGroupIsAlreadyInUse")
                        .description("system role is already in use").code(222).field("username")
    ...
    public RoleResponseDTO update(@NonNull Long id, @NonNull RoleAddRequestDTO roleAddRequestDTO) {
        ...
        try {
            transactionUtil.openTransaction();
            var systemGroupOptional = roleRepository.findById(id);
            if (systemGroupOptional.isEmpty()) {
                log.info("Group Entity id = {} not found", id);
                throw BusinessException.builder().name("role user").message("err.roleNotFound")
                        .description("Group Not Found").code(888).field("id").value(id).build();
            }
            var roleEntity = systemGroupOptional.get();
            ...
            if (roleAddRequestDTO.permissionsId() != null && !roleAddRequestDTO.permissionsId().isEmpty()) {
                ...
                for (Long permissionId : roleAddRequestDTO.permissionsId()) {
                    var permission = systemPermissionRepository.findById(permissionId);
