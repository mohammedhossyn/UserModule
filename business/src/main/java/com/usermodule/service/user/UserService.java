...
public class UserService {
    ...
    public PageResponseWithHeaderDTO<UserSearchResponseDTO> search(Pageable pageable, UserSearchRequestDTO userSearchRequestDTO) {
        ...
        var systemUserEntity = dtoUserListResponseMapper.apply(userSearchRequestDTO);
        Page<UserSearchResponseDTO> pages = userRepository.search(pageable, systemUserEntity).map(userSearchDTOMapper);
    ...
    public ForgetPasswordResponseDTO forgetPassword(ForgetPasswordRequestDTO forgetPasswordRequestDTO){
       ...
       var systemUser = userRepository.findByUsername(forgetPasswordRequestDTO.username()).orElseThrow();
        var message = messageService.sendMessageOfForgetPassword(code, systemUser.getMobileNumber());
    ...
    public ChangePasswordResponseDTO changePassword(ChangePasswordRequestDTO changePasswordRequestDTO) {
        ...
        var systemUser = userRepository.findByUsername(changePasswordRequestDTO.username()).orElseThrow();
        var forgetPasswordCodeExists = forgetPasswordCodeRepository
                .findByUsernameAndCode(systemUser.getUsername(), changePasswordRequestDTO.code(), getMinutesAgo(forgetPasswordCodeTimeout));
        if (forgetPasswordCodeExists.isPresent()){
            ...
            systemUser.setPassword(customPasswordEncoder.encode(changePasswordRequestDTO.newPassword()));
            userCrudService.update(systemUser);
            log.debug("UserService.changePassword ended");
            return ChangePasswordResponseDTO.builder().username(systemUser.getUsername()).build();
        } else {
            return null;
        }
    }
