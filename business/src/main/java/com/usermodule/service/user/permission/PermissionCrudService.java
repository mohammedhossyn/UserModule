...
public class PermissionCrudService {
    ...
    public PermissionResponseDTO add(PermissionAddRequestDTO permissionAddRequestDTO) {
        ...
        try {
            ...
            if (!permissionRepository.existsByCode(permissionAddRequestDTO.code())) {
                ...
                throw BusinessException.builder().name("add action").message("err.systemActionIsAlreadyInUse")
                        .description("system action is already in use").code(999).field("code")
    ...
    public PermissionResponseDTO update(Long id, PermissionAddRequestDTO permissionAddRequestDTO) {
        ...
        var systemActionOptional = permissionRepository.findById(id);
        if (systemActionOptional.isEmpty()) {
            log.info("Action Entity id = {} not found", id);
            throw BusinessException.builder()
                    .name("action user")
                    .message("err.actionNotFound")
                    .description("Action Not Found").code(888).field("id").value(id).build();
        }
        var permissionEntity = systemActionOptional.get();

        boolean changes = false;

        if (!permissionEntity.getCode().equals(permissionAddRequestDTO.code())) {
            if (permissionRepository.existsByCode(permissionAddRequestDTO.code())) {
                log.info("action code {} is already in use", permissionAddRequestDTO.code());
                throw BusinessException.builder().name("update action").message("err.actionCodeIsAlreadyInUse")
                        .description("action code is already in use").code(88888).field("code")
        ...
        if (!permissionEntity.getDescription().equals(permissionAddRequestDTO.description())) {
            if (permissionRepository.existsByDescription(permissionAddRequestDTO.description())) {
                log.info("action description {} is already in use", permissionAddRequestDTO.description());
                throw BusinessException.builder().name("update action").message("err.actionDescriptionIsAlreadyInUse")
                        .description("action description is already in use").code(88888).field("description")
