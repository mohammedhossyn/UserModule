...
public class UserCrudService {
    ...
    public UserResponseDTO add(UserAddRequestDTO userAddRequestDTO) {
        ...
        try {
            ...
            var systemUserEntity = userAddDTOMapper.apply(userAddRequestDTO);
            Set<ConstraintViolation<UserEntity>> violations = validator.validate(systemUserEntity);
            ...
            systemUserEntity.setUserId(null);
            systemUserEntity.setPassword(customPasswordEncoder.encode(systemUserEntity.getPassword()));
            ...
            userRolePermissionEntity.setUser(systemUserEntity);
            systemUserEntity.setUserRolePermissions(new HashSet<>());
            systemUserEntity.getUserRolePermissions().add(userRolePermissionEntity);
            var savedUser = userRepository.save(systemUserEntity);
    ...
    private UserRolePermissionEntity createUserRolePermission(UserAddRequestDTO userAddRequestDTO) {
        ...
        Long systemRoleId = userAddRequestDTO.roleId();
        RoleEntity roleEntity = roleRepository.findById(systemRoleId).orElseThrow();
        ...
        if (userAddRequestDTO.permissions() != null) {
            ...
            for (Long systemPermissionId : userAddRequestDTO.permissions()) {
                for (PermissionEntity permissionEntity : permissionEntities) {
                    if (permissionEntity.getPermissionId().equals(systemPermissionId)) {
                        log.debug("systemPermissionId : {}", systemPermissionId);
    ...
    public UserEntity addWithSpecificRole(UserEntity user, String roleName) {
        ...
        var systemRoleEntity = roleRepository.findByName(roleName).orElseThrow();
        var userRolePermissionEntity = UserRolePermissionEntity.builder()
                .role(systemRoleEntity).build();
        userRolePermissionEntity.setPermissions(new HashSet<>(systemRoleEntity.getPermissions()));
