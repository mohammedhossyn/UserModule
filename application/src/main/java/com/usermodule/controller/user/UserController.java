...
public class UserController {
    ...
    @PreAuthorize("hasAuthority('Manager') or hasAuthority('User Management') or hasAuthority('Add Employer')")
    public ApiResponseDTO addEmployer(@Valid @RequestBody UserCompanyRequestDTO userCompanyRequestDTO) {
        var employerResponseDTO = userService.addEmployer(userCompanyRequestDTO);
        return apiResponseInspector.apiResponseBuilder(employerResponseDTO, "");
    }

    @PostMapping("/add/producer")
    @PreAuthorize("hasAuthority('Employer') or hasAuthority('User Management') or hasAuthority('Add Producer')")
    public ApiResponseDTO addProducer(@Valid @RequestBody UserCompanyRequestDTO userCompanyRequestDTO) {
        var employerResponseDTO = userService.addProducer(userCompanyRequestDTO);
        return apiResponseInspector.apiResponseBuilder(employerResponseDTO, "");
    }
...
//    }
    ...
    @PostMapping("/accessToCity/employer")
    @PreAuthorize("hasAuthority('Manager') or hasAuthority('User Management') or hasAuthority('Access Employer To City')")
    public ApiResponseDTO accessCompanyToCity(@NonNull @RequestBody UserCityAddRequestDTO userCityAddRequestDTO) {
        var userCityResponseDTO = userCityService.createOrUpdate(userCityAddRequestDTO);
        return apiResponseInspector.apiResponseBuilder(userCityResponseDTO, "");
    }
}